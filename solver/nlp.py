...
class NLP(OCP):
    ...
    def solve(self, x0, xG):
        ...
        try:
            sol = self.solver_nominal(x0=y0, p=np.concatenate([x0, xG]), lbg=lbg, ubg=ubg)
    ...
    def initialize_solver(self, obstacles):
        ...
        f = (Z[:, -1] - goal).T @ self.Qf @ (Z[:, -1] - goal)
        for i in range(N):
            f += (Z[:, i] - goal).T @ self.Q @ (Z[:, i] - goal) + V[:, i].T @ self.R @ V[:, i]
